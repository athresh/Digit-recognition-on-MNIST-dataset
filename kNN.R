...
for(i in 1:numiter){
  mdl_cv<- knn(trainSub_nzv[2:dim(trainSub_nzv)[2]],cvset_nzv[2:dim(cvset_nzv)[2]],trainSub_nzv$label,k=i)
  accuracycv[i]<-sum(mdl_cv==cvset_nzv$label)/dim(cvset_nzv)[1]
  print(paste("Accuracy on CV for k=:",i,"is",accuracycv[i]))
  print("-------------------------------------------------------------")
}
print("-------------------------------------------------------------")
print(paste("Optimal value of k for kNN is",which.max(accuracycv)))
...
results=data.frame(ImageId=1:dim(testing_nzv)[1],Label=mdl_test)
write.csv(results,file="submission_knn2.csv",row.names=FALSE)
